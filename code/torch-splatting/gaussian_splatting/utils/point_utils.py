...
def get_point_clouds(cameras, depths, alphas, rgbs=None, num_layers=1, layer_dist=0.01):
    """
    ...
    base_depths = depths.flatten(1).unsqueeze(-1)
    
    all_coords = []
    for i in range(num_layers):
        # Add layers inwards by increasing depth along the ray
        d = base_depths + i * layer_dist
        pts = rays_o + rays_d * d
        all_coords.append(pts[mask].cpu().numpy())
        
    coords = np.concatenate(all_coords, axis=0)

    if rgbs is not None:
        rgbas = torch.cat([rgbs, alphas.unsqueeze(-1)], dim=-1)
        # Extract valid colors
        valid_rgbas = rgbas.flatten(1,-2)[mask].cpu().numpy()
        # Repeat colors for all layers
        rgbas_all = np.tile(valid_rgbas, (num_layers, 1))
