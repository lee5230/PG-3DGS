...
def build_rotation(r):
    # Guard against zero-norm or non-finite quaternions to avoid NaNs.
    norm = torch.linalg.norm(r, dim=1).clamp_min(1.0e-8)
    q = r / norm[:, None]
...
class GaussRenderer(nn.Module):
    ...
    def __init__(self, active_sh_degree=3, white_bkgd=True, device='cuda', **kwargs):
        ...
        self.pix_coord = torch.stack(torch.meshgrid(torch.arange(256), torch.arange(256), indexing='xy'), dim=-1).to(device)
        self.device = device
    ...
    def render(
        ...
        *,
        ...
        outline_alpha: float = 0.2,
        outline_color: torch.Tensor | None = None,
    ):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                if use_pdf:
                    gauss_weight = torch.exp(-0.5 * quad)
                    outline_mask = None
                else:
                    gauss_weight = (quad <= (oval_std ** 2)).float()
                    if outline_thickness > 0.0:
                        dist = torch.sqrt(quad.clamp(min=0.0) + 1e-8)
                        outline_mask = (dist >= (oval_std - outline_thickness)) & (dist <= (oval_std + outline_thickness))
                    else:
                        outline_mask = None
                
                alpha = (gauss_weight[..., None] * sorted_opacity[None]).clip(max=0.99) # B P 1
                # alpha = (gauss_weight[..., None]).clip(max=0.99) # B P 1
                ...
                if outline_mask is not None:
                    if outline_color is None:
                        outline_color = torch.tensor([0.0, 0.0, 0.0], device=tile_color.device)
                    outline_strength = (outline_mask.float() * sorted_opacity[None, :, 0]).max(dim=1, keepdim=True).values
                    outline_strength = outline_strength.clamp(0.0, 1.0) * outline_alpha
                    tile_color = tile_color * (1.0 - outline_strength) + outline_strength * outline_color
    ...
    def gaussian_visualize(self, camera, pc, **kwargs):
        means3D = pc.get_xyz
        opacity = pc.get_opacity
        scales = pc.get_scaling
        rotations = pc.get_rotation
        ...
        with prof("render"):
            rets = self.render(
                ...
                depths=depths,
                use_pdf=False,
                outline_thickness=kwargs.get("outline_thickness", 0.0),
                outline_alpha=kwargs.get("outline_alpha", 0.2),
