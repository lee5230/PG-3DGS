...
class GaussModel(nn.Module):
    ...
    * Attributes
    ...
    def save_to_disk(self, path):
        means = self.get_xyz.detach().cpu()           # (N, 3)
        # covs = self.get_actual_covariance().detach().cpu()   # (N, 3, 3)
        scaling = self.get_scaling.detach().cpu()     # (N, 3)
        rotation = self.get_rotation.detach().cpu()   # (N
        opacities = self.get_opacity.detach().cpu()     # (N, 1)
        features = self.get_features.detach().cpu() # (N, C, D)
        torch.save({
            "means": means,
            "scaling": scaling,
            "rotation": rotation,
    ...
    def get_actual_covariance(self, scaling_modifier=1):
        """
        Returns the actual covariance matrix of the gaussians.
        This is computed from the scaling and rotation parameters.
        """
    ...
    def __init__(self, sh_degree : int=3, debug=False):
        ...
        # self._opacity = nn.Parameter(torch.ones(1, 1))
